...
class DashboardService:
    ...
    def get_stats(self, current_user):
        ...
        if is_employee and emp:
            ...
            # Count employees who selected "Will have lunch tomorrow" during today's check-in
            lunch_count_today = self.db.query(Attendance).filter(
                Attendance.date == today,
                Attendance.employee_id == emp.id,
                Attendance.auto_lunch_counted.is_(True),
            ).count()
        else:
            present_today = self.db.query(Attendance).filter(
                Attendance.date == today,
        ...
        return {
            ...
            "lunch_count_today": lunch_count_today,
